    ...
    public class Hid : object
    {
        ...
        private void ReadCompleted(IAsyncResult iResult)
        {
            ...
            try
            {
                ...
                Debug.PrintLog("ReadCompleted: len=" , e.reportBuff.Length.ToString());
        ...
        public HID_RETURN Write(report r)
        {
            if (deviceOpened)
            {
                try
                {
                    ...
                    uint outLen = 0;
                    //  WriteFile(hHubDevice, buffer, (uint)buffer.Length,out outLen, IntPtr.Zero);
                    /// hidDevice.BeginWrite(buffer,0, buffer.Length,null, buffer);
                    hidDevice.Write(buffer, 0, OutputReportLength);
                    Debug.PrintLog("写数据", buffer);
                    return HID_RETURN.SUCCESS;
                }
                catch (Exception ex1)
                {
